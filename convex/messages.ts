...
export const getById = query({
    ...
    handler: async (ctx, args) => {
        ...
        const dedupedReactions = reatcionsWithCount.reduce(
            (acc, reaction) => {
                const exitsingReaction = acc.find((r) => r.value === reaction.value);
                if (exitsingReaction) {
                    exitsingReaction.memberIds = Array.from(
                        new Set([...exitsingReaction.memberIds, reaction.memberId])
                    )
                } else {
